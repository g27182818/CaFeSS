...
while True:
    ...
    if((var == False and contador > 0) and act == False):
        ...
        act = True
    diftime = 30
    if (((((int(time.time()-instanteInicial))/60) > (diftime+0.1)) or contador == 0) and arranque == True):
    ...
    if ((((time.time()-instanteInicial)/60) > diftime-0.1) and var == True):
        window.Element('lectura').Update('En lectura')
    if (((((time.time()-instanteInicial))/60) > diftime-0.07) and var == True):
        print("Leyendo")
        # lecturadatos()
